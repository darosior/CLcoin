...
#if defined(NDEBUG)
# error "CLcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe930a7b101831a11c75ca33fa565a31c340950b2ab20dc5ebb6befa32c4f6d1b");
...
const string strMessageMagic = "CLcoin Signed Message:\n";
...
{
    // CLcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // CLcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // CLcoin: 3.5 Days (same as Litecoin)
static const int64 nTargetSpacing = 2.5 * 60 ; // (Like Litecoin) CLcoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CLcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe930a7b101831a11c75ca33fa565a31c340950b2ab20dc5ebb6befa32c4f6d1b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Darosior 04/05/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 420 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042a1ac0f0ad4d95f74739f7b3c073f9c9cefada8dc86b46c218e4a99a7b0933b0b5ab5329a979b1c36ea326567b6f2083a5e26f0522db408f3f20a1c18d1d1850") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525621792;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087959445;
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb1, 0xdb }; // CLcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CLcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CLcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CLcoinMiner(CWallet *pwallet)
{
    printf("CLcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CLcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CLcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CLcoinMiner, pwallet));
}
